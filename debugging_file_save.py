...
class RadioRecorder:
    ...
    def _record_loop(self):
        ...
        while ((not self.stop_flag) or (len(sample_blocks) < MIN_BLOCKS)) and (time.time() - start_time) < 90:
            block = self.sdr.read_samples(block_size)
            print("DEBUG: block size returned =", len(block))
            sample_blocks.append(block)
        
        print("DEBUG: finished read loop.")
        print("DEBUG: samples collected=",len(sample_blocks))

        if sample_blocks:
            self.samples = np.concatenate(sample_blocks)
            print("DEBUG: Total samples after concatenating=", self.samples.size)
        else:
        ...
        if self.samples.size > 0:
            ...
            print("DEBUG: Preparing to save NPZ file...")
            print("DEBUG: Outputh path=", output_path)

            # save samples to file human decoding
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = f"/home/astrobotic/Desktop/Controls/data/sdr_record_{ts}.npz"
            try:
            ...
            if temperatures_f:
                temp_path = f"/home/astrobotic/Desktop/Controls/data/temps_{ts}.npz"
                try:
    ...
    def stop(self):
        if not self.recording:
            ...
            return
        print("RadioRecorder: Stopping thread...")
        self.stop_flag = True

        # exit SDR loop naturally
        if self.thread is not None:
            self.thread.join(timeout=3)

        self.recording = False
        print("RadioRecorder: stopped cleanly.")
...
class BaseController(Controller):
    ...
    def on_circle_press(self):
        ...
        print("                                           ")
        print("                                           ")
        print("BYEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE")
        radio_recorder.stop()
